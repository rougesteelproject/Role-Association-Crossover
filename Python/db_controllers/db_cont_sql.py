...
class DatabaseControllerSQL():
    ...
    def _execute(self, command, parameters = None):
        self._sql_cursor._execute(command, parameters)

        self._sql_connection.commit()

    def _create_db_if_not_exists(self):
        # Create table if it doesn't exist
        self._execute('''CREATE TABLE IF NOT EXISTS meta_roles(id INTEGER PRIMARY KEY, name TEXT NOT NULL, description TEXT DEFAULT \'Auto-Generated\', historical TEXT DEFAULT \'False\', religious TEXT DEFAULT \'False\', fictional_in_universe TEXT DEFAULT \'False\',is_biggest TEXT DEFAULT \'False\')''')
        ...
        self._execute('''CREATE TABLE IF NOT EXISTS actors_to_abilities(actor_id INT, ability_id INT, UNIQUE(actor_id, ability_id))''')
        self._execute('''CREATE TABLE IF NOT EXISTS ability_templates_to_abilities(template_id INT, ability_id INT, UNIQUE(template_id,ability_id))''') #[species/power source: ability_id]
        self._execute('''CREATE TABLE IF NOT EXISTS ability_templates(template_id INTEGER PRIMARY KEY, template_name TEXT, template_description TEXT)''')
        self._execute('''CREATE TABLE IF NOT EXISTS roles_to_ability_templates(role_id TEXT, template_id TEXT, UNIQUE (role_id, template_id))''')
        self._execute('''CREATE TABLE IF NOT EXISTS roles_to_abilities(role_id TEXT, ability_id TEXT, UNIQUE (role_id, ability_id))''')
        self._execute('''CREATE TABLE IF NOT EXISTS abilities(id INTEGER PRIMARY KEY, name TEXT, description TEXT)''') #[krypt: strength (kyptonian): kryptonians can lift quintillion tons blah blah]
        #create history tables
        self._execute('''CREATE TABLE IF NOT EXISTS meta_roles_history(id INT NOT NULL, timestamp TEXT DEFAULT CURRENT_TIMESTAMP, name TEXT NOT NULL, description TEXT, historical TEXT, religious TEXT, fictional_in_universe TEXT, UNIQUE(id, name, description, historical, religious, fictional_in_universe))''')
        self._execute('''CREATE TABLE IF NOT EXISTS roles_history(id TEXT NOT NULL, timestamp TEXT DEFAULT CURRENT_TIMESTAMP, name TEXT NOT NULL, description TEXT, alive_or_dead TEXT, alignment TEXT, UNIQUE (id, name, description, alive_or_dead, alignment))''')
        self._execute('''CREATE TABLE IF NOT EXISTS actors_history(id INT NOT NULL, timestamp TEXT DEFAULT CURRENT_TIMESTAMP, name TEXT NOT NULL, bio TEXT, UNIQUE(id, name, bio))''')
        #history tables for abilities
        self._execute('''CREATE TABLE IF NOT EXISTS abilities_history(id INT NOT NULL, timestamp TEXT DEFAULT CURRENT_TIMESTAMP, name TEXT, description TEXT, UNIQUE(id, name, description))''')
        self._execute('''CREATE TABLE IF NOT EXISTS ability_template_history(id, INT NOT NULL, timestamp TEXT DEFAULT CURRENT_TIMESTAMP, name TEXT NOT NULL, description TEXT, UNIQUE(id, name, description))''')
    ...
    def create_actor(self, id, name, bio, birth_date, death_date):
        ...
        self._execute(create_actor_sql, (id, name, bio, birth_date, death_date,))
    ...
    def create_mr(self,character_name, mr_id):
        ...
        self._execute(create_mr_sql,(character_name, mr_id))
    ...
    def update(self, table_name, column, column_value, where, where_value):
        ...
        self._execute(update_sql, (column_value, where_value,))
    ...
    def update_or(self, table_name, column, column_value, where_1, where_value_1, where_2, where_value_2):
        ...
        self._execute(update_sql, (column_value,where_value_1,where_value_2,))
    ...
    def select_where(self, select_columns, table_name, where, where_value):
        ...
        self._execute(select_sql, (where_value,))
    ...
    def select_and(self, select_columns, table_name, where_column, where_value, where_column_2, where_value_2):
        ...
        self._execute(select_sql,(where_value,where_value_2,))
        return self.cursor.fetchall()
    ...
    def select_not_in(self, select_columns, table_name, where, ability_list):
        result_set = self._execute("SELECT {} FROM {} WHERE {} NOT IN ".format(select_columns.lower(),table_name.lower(),where.lower()) + '(%s)' % ','.join('?'*len(ability_list)), ability_list)
    ...
    def add_image(self,page_type, page_id, image_url, caption):
        ...
        self._execute(sql,(image_url,page_id,caption,))
    ...
    def create_actor_history(self, id, new_bio):
        ...
        self._execute(historySql,(id, old_actor.name, old_actor.bio))
        
        changeDescSql='''UPDATE actors SET bio=? WHERE id=?'''
        self._execute(changeDescSql,(new_bio,id,))
    ...
    def create_mr_history(self, id, new_description, historical, religious, fictional_in_universe):
        ...
        self._execute(historySql,(id, old_mr.name, old_mr.description, old_mr.historical, old_mr.religious, old_mr.fictional_in_universe))
        changeDescSql='''UPDATE meta_roles SET description=?, historical=?, religious=?, fictional_in_universe=? WHERE id=?'''

        self._execute(changeDescSql,(new_description,historical, religious, fictional_in_universe,id,))
    ...
    def create_role_history(self, id, new_description, new_alive_or_dead, new_alignment):
        ...
        self._execute(historySql,(id, old_role.name, old_role.description, old_role.alive_or_dead, old_role.alignment))
        changeDescSql='''UPDATE roles SET description=?, alive_or_dead=?,alignment=? WHERE id=?'''

        self._execute(changeDescSql,(new_description,new_alive_or_dead,new_alignment,id,))  
    ...
    def create_ability(self, name, description):
        ...
        self._execute(create_sql, (name, description))
    ...
    def create_ability_history(self, id, name, description):
        ...
        try:
            ...
            self._execute(historySql, (old_ability.id, old_ability.name, old_ability.description,))
        except IntegrityError:
            logging.exception()

        changeDescSql='''UPDATE abilities SET name=?, description=? WHERE id=?'''
        self._execute(changeDescSql, (name, description, id,))

    def remove_ability_actor(self, actor_id, ability_list):
        self._execute("DELETE FROM actors_to_abilities WHERE actor_id={}  AND ability_id IN ".format(actor_id) + '(%s)' % ','.join('?'*len(ability_list)), ability_list)

    def remove_ability_role(self, role_id, ability_list):
        self._execute("DELETE FROM roles_to_abilities WHERE role_id={}  AND ability_id IN ".format(role_id) + '(%s)' % ','.join('?'*len(ability_list)), ability_list)
    ...
    def add_ability_actor(self, actor_id, ability_list):
        ...
        for ability_id in ability_list:
            self._execute(create_ability_actor_sql,(actor_id,ability_id,))
    ...
    def add_ability_role(self, role_id, ability_list):
        ...
        for ability_id in ability_list:
            self._execute(create_ability_role_sql,(role_id,ability_id,))
    ...
    def create_ability_template(self, name, description):
        ...
        self._execute(create_template_sql,(name,description))
        return self.cursor.lastrowid

    def remove_template(self, role_id, template_id_list):
        self._execute("DELETE FROM roles_to_ability_templates WHERE role_id={}  AND ability_id IN ".format(role_id) + '(%s)' % ','.join('?'*len(template_id_list)), template_id_list)
    ...
    def add_template_role(self, role_id, template_id_list):
        ...
        for template_id in template_id_list:
            self._execute(add_template_sql, (role_id, template_id))
    ...
    def remove_ability_from_template(self, template_id, ability_list):
        self._execute("DELETE FROM ability_templates_to_abilities WHERE template_id={} AND ability_id IN".format(template_id) + '(%s)' % ','.join('?'*len(ability_list)), ability_list)
    ...
    def add_abilities_to_template(self,template_id, ability_list):
        ...
        for ability_id in ability_list:
            self._execute(connect_template_ability_sql,(template_id,ability_id,))
    ...
    def create_template_history(self, template_id, new_name, new_description):
        ...
        try:
            ...
            self._execute(historySql, (old_template.id, old_template.name, old_template.description,))
        except IntegrityError:
            logging.exception()

        changeDescSql='''UPDATE ability_templates SET template_name=?, template_description=? WHERE template_id=?'''
        self._execute(changeDescSql, (new_name, new_description, template_id,))
    ...
    def add_relationship_actor(self, actor1_id, actor1_name, actor2_id, actor2_name, relationship_type):
        ...
        self._execute(sql, (actor1_id, actor1_name, actor2_id, actor2_name, relationship_type))
    ...
    def add_relationship_role(self, role1_id, role1_name, role2_id, role2_name, relationship_type):
        ...
        self._execute(sql, (role1_id, role1_name, role2_id, role2_name, relationship_type))
